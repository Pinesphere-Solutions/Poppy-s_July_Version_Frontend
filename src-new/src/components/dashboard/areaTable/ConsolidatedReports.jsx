...
const API_URL = "http://127.0.0.1:8000/api/get_consolidated_logs/";
  ...
  const downloadSummaryCSV = () => {
    ...
    if (summaryFilter === "operator") {
      ...
      nonSummaryData.forEach((item) => {
        ...
        if (
          item.MODE === 1 &&
          item.RESERVE !== undefined &&
          item.RESERVE !== null
        ) {
          const reserve = Number(item.RESERVE) || 0;
          if (reserve > 0) {
            // Only count meaningful sewing speeds
      ...
      const totalProductiveHours = operators.reduce(
        (sum, op) => sum + op.sewingHours,
        0
      );
      const totalHours = operators.reduce((sum, op) => sum + op.totalHours, 0);
      const totalNeedleRuntime = operators.reduce(
        (sum, op) => sum + op.needleRuntime,
        0
      );
      const totalSewingSeconds = operators.reduce(
        (sum, op) => sum + op.sewingSeconds,
        0
      );
      ...
      const totalSewingSpeed = operators.reduce((sum, op) => {
        const speed =
          op.numberOfInstances > 0 ? op.totalSPM / op.numberOfInstances : 0;
        return sum + speed;
      }, 0);

      return {
        productiveTime: totalProductiveHours, // CHANGED: Show total hours instead of percentage
        needleRuntimePercentage:
          totalSewingSeconds > 0
            ? (totalNeedleRuntime / totalSewingSeconds) * 100
            : 0, // Keep as percentage but calculated from totals
    ...
    if (summaryFilter === "machine") {
      ...
      csvContent +=
        "Machine ID,Total Hours,Sewing,Idle,Meeting,No Feeding,Maintenance,Rework,Needle Break,Productive %,Non-Productive %\n";

      [
        ...new Set(
          filteredData.filter((d) => !d.isSummary).map((d) => d.MACHINE_ID)
        ),
      ].forEach((machineId) => {
        ...
        const totalHours =
          ...
          maintenance +
          rework +
          needleBreak; // Replace these calculations in createSummaryRow function
        ...
        csvContent += `"${machineId}","${formatHoursMinutes(
          totalHours
        )}","${formatHoursMinutes(sewing)}","${formatHoursMinutes(
          idle
        )}","${formatHoursMinutes(meeting)}","${formatHoursMinutes(
          noFeeding
        )}","${formatHoursMinutes(maintenance)}","${formatHoursMinutes(
          rework
        )}","${formatHoursMinutes(
          needleBreak
        )}","${productivePercent}%","${nonProductivePercent}%"\n`;
      });
      csvContent += "\n";
    }
    ...
    if (summaryFilter === "line") {
      ...
      csvContent +=
        "Line Number,Total Hours,Sewing,Idle,Meeting,No Feeding,Maintenance,Rework,Needle Break,Productive %,Non-Productive %\n";

      [
        ...new Set(
          filteredData.filter((d) => !d.isSummary).map((d) => d.LINE_NUMB)
        ),
      ].forEach((lineNum) => {
        ...
        csvContent += `"${lineNum}","${formatHoursMinutes(
          totalHours
        )}","${formatHoursMinutes(sewing)}","${formatHoursMinutes(
          idle
        )}","${formatHoursMinutes(meeting)}","${formatHoursMinutes(
    ...
    csvContent += `"Productive Time","${calculateAggregatedMetrics().productiveTime.toFixed(
      2
    )}%"\n`;
    csvContent += `"Needle Runtime Percentage","${calculateAggregatedMetrics().needleRuntimePercentage.toFixed(
      2
    )}%"\n`;
    csvContent += `"Sewing Speed","${calculateAggregatedMetrics().sewingSpeed.toFixed(
      0
    )}"\n`;
    csvContent += `"Total Hours","${formatHoursMinutes(
